...
namespace BetsySignThing
{
    public static class Utilities
    {
        ...
        [Obsolete("This is deprecated, please use FromFixedPointSigned() instead.")]
        private static double FromFixedPoint(byte[] contents, int wordoffset, int bitoffset, double resolution)
        {
        ...
        public static Int32 Get24BitsSigned(byte[] contents, int wordoffset)
        {
            ...
            UInt32 raw = Get24BitsUnsigned(contents, wordoffset);

            // 2. (Trick) if bit 24 is set, force the upper 8 bits high so C# sees it as a signed int.
            if ((raw & 0x800000) != 0)
            {
                raw = (UInt32)(raw | 0xFF000000);   // 0b11111111000000000000000000000000
            }

            // 3. Force to signed
            var retsigned = (Int32)raw;
        ...
        /// <summary>
        /// Extract an ASCII string from the message.
        /// </summary>
        ...
        public static string GetASCII(byte[] contents, int wordoffset, int bitoffset, int digits)
        {
            if ((bitoffset % 8) != 0)
            {
                throw new ArgumentException("Bit offset must be divisible by 8", "bitoffset");
            }

            int offset = (wordoffset*4) + (bitoffset / 8);            
            String temp = Encoding.ASCII.GetString(contents, offset, digits);

            // Strip out possible nulls
        ...
        */
        ...
        /// <summary>
        /// Read all bytes from a file.  Report any exceptions in a dialog box.
        /// </summary>
        /// <param name="filename">Filename</param>
        /// <returns>byte array of file or null if exception</returns>
        public static byte[] ReadAllBytes(String filename)
        {
            ...
            {
                MessageBox.Show("Exception reading binary file:" + ex.Message);
                return null;
            }
        }

        /// <summary>
        /// Load a binary log file after prompting in a dialog box.
        /// </summary>
        /// <param name="defaultpath">Default path for loading from.</param>
        /// <returns>Array of bytes read from file, or null on Error or Cancel</returns>
        public static byte[] LoadBinary(String defaultpath, String filter)
        {
            ...
            {
                return ReadAllBytes(openFileDialog1.FileName);
            }
            else
            {
                return null;
        ...
        public static byte[] LoadBinary(String defaultpath)
        {
            return LoadBinary(defaultpath, "All files (*.*)|*.*");
        }
    }
}